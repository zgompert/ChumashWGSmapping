...
```
Related to the above, I also asked whether LD among the color associated regions increased in the survivors on AC relative to the starting conditions, and likewise, whether this was not true for MM. This analysis is in [chnagePCAGwa.R](changePCAGwa.R), which is below and in `/uufs/chpc.utah.edu/common/home/gompert-group5/projects/t_chum_mapping/gemma` on the cluster. For this, I combined the P-values (log10) from the single SNP association mapping for RG and GB and then defined regions based on the set of SNPs with log10 P > 10. The exact defintions of the regions was somewhat subjective as I decided to pool into the three main regions. I will revisit this after we have the *T. chumash* genomes. Nonetheless, I think what I have is easy enough to justify and that anything remotely reasonable would capture the same pattern. I then ran a PCA (centered but not standardized genotypes) for the experimental *T. chumash* for each of the three CH8 regions. I saved the first two PCs from each region and looked at correlations among these in the initial set and survivors and compared the difference to null expectations based on random survival. We see a substantial and significant increase in correlations (LD) for AC but not for MM (details in code below).
